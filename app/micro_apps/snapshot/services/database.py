...
class DataBase:
    ...
    def get_root_id(self, snapshot_name):
        ...
        return self._db.find_document(file_snapshot_collection_name, query)["root_id"]
    ...
    def get_file_under_folder(
        ...
        if folder_id:
            query = {"$match": {"parents": {"$in": [folder_id]}}}
        else:
            query = {"$match": {"parents": {"$size": 0}}}
    ...
    def update_path_and_permissions(self, snapshot_name, folder_path, folder_permission, file_id):
        new_path = self.update_path(snapshot_name, folder_path, file_id)
        new_permissions = self.update_permissions_to_inherit_direct(snapshot_name, folder_permission, folder_path,
                                                                    file_id)
        return new_path, new_permissions

    def update_permissions_to_inherit_direct(self, snapshot_name, parent_path, parent_permissions, file_id):
        permission_collection_name = f"{self.user_id}.{snapshot_name}.permissions"
        ...
        permissions = self._db.find_documents(permission_collection_name, query, {"_id": 0})
        return permissions
    ...
    def get_all_permission_of_file(self, snapshot_name, file_id):
        ...
        query = {"file_id": file_id}
        filter_query = {"_id": 0}
        permissions = self._db.find_documents(permission_collection_name, query, filter_query)
        return permissions
